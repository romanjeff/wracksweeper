...
int main(int argc, char * argv[], char * envp[])
{
	...
	std::string filename;
	int index = 0, i = 0;
	uint8_t range_t, freq_t, gain_t;

	// If there were no arguments, exit out of program
	if (argc == 1)
	{
		printf("ERROR: Insufficient number of arguments\nEXITING...\n");
		return 0;
	}
	// If there were not enough inputs for parameters, warn user and use default values
	else if (argc == 2 || argc == 3 || argc == 4)
	{
		printf("WARNING: Missing parameter values, default values will be used\n");
		filename = argv[1];
	}
	// All inputs for filename and parameter were met
	else if (argc == 5)
		filename = argv[1];
	...
	// If all arguments required are met, store as parameter variables
	if (argc == 5)
	{
		range_t = stoi(argv[2]);
		freq_t = stoi(argv[3]);
		...
		sonarSwitches.Range = range_t;
		sonarSwitches.Freq = freq_t;
		sonarSwitches.StartGain = gain_t;
	}
